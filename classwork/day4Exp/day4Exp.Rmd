...
plot(data$x[data$Group == 1] ~ unique(data$Time), type = 'l',
     xlab = 'Time', ylab = 'x', ylim = c(min(data$x)-.1, max(data$x) +.1),
     col = col[1])
for(i in 1:max(data$Group)) points(data$x[data$Group == i] ~ unique(data$Time), col = col[i], type = 'l')
...
plot(data$x[data$Group == 1] ~ unique(data$Time), type = 'l',
     xlab = 'Time', ylab = 'x', ylim = c(min(data$x)-.1, max(data$x) +1),
     col = col[1])
for(i in 1:max(data$Group)) points(data$x[data$Group == i] ~ unique(data$Time), col = col[i], type = 'l', lty = i)
...
for(i in 1:max(data$Group)) points(data$x[data$Group == i] ~ unique(data$Time), col = col[i], type = 'b', lty = i, pch = i, cex = .3)
legend('topleft', legend = 1:max(data$Group), col = col, lty = 1:max(data$Group), pch = 1:max(data$Group), cex = .3, bty = 'n', horiz = T)
#lets repeat for y to investiagete the auto-correlation in the outcome
plot(data$y[data$Group == 1] ~ unique(data$Time), type = 'b',
     xlab = 'Time', ylab = 'y', ylim = c(min(data$y)-.1, max(data$y) +1),
     col = col[1], pch = 1, cex = .3)
for(i in 1:max(data$Group)) points(data$y[data$Group == i] ~ unique(data$Time), col = col[i], type = 'b', lty = i, pch = i, cex = .3)
legend('topleft', legend = 1:max(data$Group), col = col, lty = 1:max(data$Group), pch = 1:max(data$Group), cex = .3, bty = 'n', horiz = T)
...
plot(data$y[data$Group == 1] ~ data$x[data$Group == 1], type = 'p',
     ...
     col = col[1], pch = 1, cex = .3)
for(i in 1:max(data$Group)) points(data$y[data$Group == i] ~ data$x[data$Group == i], col = col[i], type = 'p', lty = i, pch = i, cex = .3)
legend('topleft', legend = 1:max(data$Group), col = col,
       pch = 1:max(data$Group), cex = .3, bty = 'n', horiz = T)
#add the lines
for(i in 1:max(data$Group)) abline(a = coef(mod)[i+1], b = coef(mod)[1], col = col[i])
...
points(toAnalyze[[2]] ~ seq(0, .9, by=.1), type='l')
```
